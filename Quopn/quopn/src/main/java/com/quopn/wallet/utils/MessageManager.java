...
class MessageManager extends Handler implements Comparator<AppMessage> {
    ...
    private final Queue<AppMessage> msgQueue;
    private final Queue<AppMessage> stickyQueue;

    private MessageManager() {
        msgQueue = new PriorityQueue<AppMessage>(1, this);
        stickyQueue = new LinkedList<AppMessage>();
    }

    /**
     * @return A {@link MessageManager} instance to be used for given {@link android.app.Activity}.
     */
    static synchronized MessageManager obtain(Activity activity) {
        if (sManagers == null) {
            sManagers = new WeakHashMap<Activity, MessageManager>(1);
        }
        MessageManager manager = sManagers.get(activity);
        if (manager == null) {
    ...
    static synchronized void release(Activity activity) {
        if (sManagers != null) {
            final MessageManager manager = sManagers.remove(activity);
            if (manager != null) {
                manager.clearAllMsg();
            }
        }
    ...
    static synchronized void clearAll() {
        if (sManagers != null) {
            final Iterator<MessageManager> iterator = sManagers.values().iterator();
            while (iterator.hasNext()) {
                final MessageManager manager = iterator.next();
                if (manager != null) {
                    manager.clearAllMsg();
                }
                iterator.remove();
    ...
    /**
     * Inserts a {@link AppMessage} to be displayed.
     *
     * @param appMsg
     */
    void add(AppMessage appMsg) {
        msgQueue.add(appMsg);
    ...
    /**
     * Removes all {@link AppMessage} from the queue.
     */
    void clearMsg(AppMessage appMsg) {
    ...
    void clearShowing() {
        final Collection<AppMessage> showing = new HashSet<AppMessage>();
    ...
    /**
     * Displays the next {@link AppMessage} within the queue.
     */
    private void displayMsg() {
        ...
        final AppMessage appMsg = msgQueue.peek();
        final Message msg;
        if (!appMsg.isShowing()) {
            // Display the AppMsg
            ...
            msg.obj = appMsg;
            sendMessage(msg);
        } else if (appMsg.getDuration() != AppMessage.LENGTH_STICKY) {
    ...
    /**
     * Removes the {@link AppMessage}'s view after it's display duration.
     *
     * @param appMsg The {@link AppMessage} added to a {@link ViewGroup} and should be removed.s
     */
    private void removeMsg(final AppMessage appMsg) {
        clearMsg(appMsg);
    ...
    private void addMsgToView(AppMessage appMsg) {
        ...
        if (duration != AppMessage.LENGTH_STICKY) {
    ...
    public void handleMessage(Message msg) {
        final AppMessage appMsg;
        switch (msg.what) {
            case MESSAGE_DISPLAY:
                displayMsg();
                break;
            case MESSAGE_ADD_VIEW:
                appMsg = (AppMessage) msg.obj;
                addMsgToView(appMsg);
                break;
            case MESSAGE_REMOVE:
                appMsg = (AppMessage) msg.obj;
    ...
    private static class OutAnimationListener implements Animation.AnimationListener {
        ...
        private final AppMessage appMsg;

        private OutAnimationListener(AppMessage appMsg) {
            this.appMsg = appMsg;
        }
